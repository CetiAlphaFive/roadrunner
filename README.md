...
`ares()` — Multivariate Adaptive Regression Splines (Friedman 1991) via
`Rcpp` + `RcppParallel`. Numerically comparable to
[`earth`](https://cran.r-project.org/package=earth) on the gaussian core,
faster on most cells at 4 threads, and parallel-deterministic
...
See the [`ares` vignette](vignettes/ares.Rmd) for autotune,
classification, weights, bagging, and prediction-interval examples.
