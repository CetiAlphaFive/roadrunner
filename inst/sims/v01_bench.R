## v0.1 benchmark: ares (post fast-LS) vs earth.
## Output: inst/sims/results/v0.1-bench.csv
...
for (i in seq_len(nrow(cells))) {
  ...
  cat(sprintf("[%2d/%d] %s n=%d deg=%d t=%d  ares=%.3fs  earth=%s  rss_relerr=%s  ratio=%s\n",
              ...
              if (is.na(rows[[i]]$speed_ratio)) "NA" else sprintf("%.1fx", rows[[i]]$speed_ratio)))
}

out <- do.call(rbind, rows)
dir.create("inst/sims/results", showWarnings = FALSE, recursive = TRUE)
write.csv(out, "inst/sims/results/v0.1-bench.csv", row.names = FALSE)

cat("\n--- speed_ratio summary @ threads=1 ---\n")
sub <- subset(out, threads == 1)
print(summary(sub[, c("t_ares", "t_earth", "rss_rel_err", "speed_ratio")]))

cat("\n--- t_ares by threads (median/cell-set) ---\n")
agg <- aggregate(t_ares ~ threads, data = out, FUN = median)
print(agg)

cat("\nWritten: inst/sims/results/v0.1-bench.csv\n")
